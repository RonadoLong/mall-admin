<template>
    <div v-loading="listLoading" class="app-container calendar-list-container ">
        ...
        <el-table :data="settingList" border fit highlight-current-row style="width: 100%">
            ...
            <el-table-column align="center" label="time/days">
                <template slot-scope="scope">
        ...
        <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
            ...
            <el-form :rules="rules" ref="dataForm" :model="temp"
                     label-position="left" label-width="70px" style='width: 400px; margin-left:50px;'>
                <el-form-item label="Time/days" prop="time">
...
<script>
    import {addPaymentSetting, delPaymentSetting, getPaymentSettingList, updatePaymentSettingStatus} from "~/api/service/service";
    ...
    export default {
        ...
        methods:{
            ...
            loadData(){
                this.listLoading = true;
                getPaymentSettingList(this.pageNum, this.pageSize).then(res => {
                   console.log(res)
            ...
            createData(){
                this.$refs['dataForm'].validate((valid) => {
                    if (valid) {
                        this.listLoading = true;
                        addPaymentSetting(this.temp).then(() => {
                            this.loadData();
            ...
            updateData(){
                this.$refs['dataForm'].validate((valid) => {
                    if (valid) {
                        ...
                        addPaymentSetting(tempData).then(() => {
                            this.loadData();
                            this.dialogFormVisible = false
                            this.$notify({
                                title: 'tip',
            ...
            handleDelete(row){
                this.$confirm('Danger action, go on?', 'Tip', {
                    ...
                    delPaymentSetting(row.id).then(res => {
                        if (res.code === 200){
                            this.$message({
                                type: 'info',
            ...
            handleUpdateStatus(row){
                updatePaymentSettingStatus(row.id).then(res=>{
                    console.log(res)
                   this.loadData()
                })
            }
