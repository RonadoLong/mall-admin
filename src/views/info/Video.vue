<template>
    <div class="app-container">
        ...
        <el-table :data="userList" v-loading="listLoading" border fit highlight-current-row style="width: 100%">
            <el-table-column fixed prop="id" label="NO" align="center" min-width="80">
            </el-table-column>
            <el-table-column prop="title" align="center" label="title" min-width="120">
            </el-table-column>

            <el-table-column prop="category" align="center" label="category" min-width="140">
            </el-table-column>

            <el-table-column align="center" label="thumb_url">
                <template slot-scope="scope">
                    <img :src="scope.row.thumb_url" style="width: 100%"/>
                </template>
            </el-table-column>
            ...
            <el-table-column prop="duration" align="center" label="duration">
            </el-table-column>
            ...
            <el-table-column label="operation" fixed="right" align="center" min-width="140">
                <template slot-scope="scope">
                    <el-button @click="handleUpdateStatus(scope.row)" type="primary" icon="el-icon-edit"
                               size="small" circle>
                    </el-button>
    ...
    export default {
        ...
        methods: {
            getList() {
                ...
                BaseAPI.getVideoList(this.currentPage, this.pageSize).then(res => {
            ...
            handleDelete( rows) {
                this.$confirm('Danger action, go on delete ?', 'Tip', {
                    ...
                    this.listLoading = true;
                    BaseAPI.delVideo(rows.id).then(res => {
                        this.listLoading = false;
                        if (res.code === 200) {
                            this.getList();
                            this.$notify({
            ...
            handleUpdateStatus(row) {
                this.listLoading = true;
                BaseAPI.videoUpdateStatus(row.id).then(res => {
                    this.listLoading = false;
                    if (res.code === 200) {
                        this.getList();
                        this.$notify({
